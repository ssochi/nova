...
39. When extending `panic`, keep the entry explicit in builtin validation plus bytecode, reuse the existing defer/frame unwind model, and treat explicit builtin `panic(...)` as a terminating path in semantic fallthrough analysis instead of forcing unreachable `return` scaffolding.
40. When extending `recover`, keep it explicit as a builtin plus VM-frame capability: only directly deferred user-defined function frames may recover, helper calls and deferred builtin `recover()` must still yield nil, and staged runtime panic payload types must stay documented when exact Go runtime objects are absent.
41. When extending empty-interface `any` / `interface{}`, keep the syntax explicit in the AST, keep boxing explicit in the checked / bytecode pipeline, and model nil-interface state distinctly from boxed typed-nil composite values instead of erasing everything into raw runtime values.
42. When extending type assertions, keep `x.(T)` explicit in the AST / checked / bytecode / VM pipeline, require interface operands in semantic analysis, preserve typed-nil composite payloads on success, keep comma-ok assertions explicit as dedicated statement/header/post forms, and keep type switches explicit as statement nodes instead of hiding either path behind generic multi-result calls or expression-switch rewrites.
43. When `run` fails during runtime execution, preserve any already-buffered program output in the user-visible diagnostic so deferred prints and other pre-failure output are not silently dropped.
