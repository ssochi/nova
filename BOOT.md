...
42. When extending type assertions, keep `x.(T)` explicit in the AST / checked / bytecode / VM pipeline, require interface operands in semantic analysis, preserve typed-nil composite payloads on success, and keep comma-ok assertions plus type switches as separate later slices instead of hiding them behind generic multi-result calls.
