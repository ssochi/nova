...
- `2026-03-20-type-switches-and-comma-ok-assertions.md`: validates staged comma-ok assertions, empty-interface type switches, explicit `type-assert-ok` bytecode, and file-size governance across the full CLI and VM stack
- `2026-03-20-type-assertions-first-slice.md`: validates staged single-result `x.(T)` assertions, explicit assertion bytecode, runtime interface-conversion failures, and file-size governance across the full CLI and VM stack
