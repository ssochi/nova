...
- String slice execution, `make`, nil, explicit backing-array allocation APIs, or byte-specialized slice builtins
...
- `src/runtime/value.rs` and `src/runtime/vm.rs`: shared slice runtime values, rendering, indexing, slice-window construction, overlap-safe copy, and capacity-aware append execution
