...
- `any` / `interface{}`
  ...
  - The current assertion slice supports explicit single-result `value.(T)` plus statement-scoped comma-ok assertions such as `value, ok := boxed.(T)` over staged runtime types
  - Successful assertions preserve typed-nil slice/map/chan payloads; failed comma-ok assertions return the zero value of `T` plus `false` instead of panicking
  - Type switches now dispatch over empty-interface operands with explicit `case nil` versus concrete-type matching, and clause bindings keep either the asserted concrete payload or the original interface value depending on clause shape
- `slice`
...
- Bytecode now also records explicit `type-assert <type>` and `type-assert-ok <type>` instructions so interface assertions remain inspectable instead of disappearing into generic helper calls
...
- Interface assertions now execute through the same dedicated runtime interface helper seam, preserving typed-nil slice/map/chan payloads on success, returning zero-value-plus-`false` for comma-ok failures, and raising staged interface-conversion panics only for the explicit single-result assertion path
