...
- Supported types are limited to `int`, `byte`, `bool`, `string`, `[]T`, `map[K]V`, and `void`, and the current type-valued syntax surface is limited to builtin `make` plus explicit `[]byte(string)` / `string([]byte)` conversions.
...
- Map support is still staged: single-result indexing, `len`, nil-map zero values, `make`, and index assignment are supported, while map literals, `delete`, comma-ok lookups, and `range` remain deferred.
