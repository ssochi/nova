...
- `2026-03-20-14-17-12-type-switches-and-comma-ok-assertions`: completed plan for staged comma-ok type assertions, empty-interface type switches, explicit `type-assert-ok` bytecode, typed-nil type-switch behavior, and file-size-governed frontend/compiler splits
...
- The strongest adjacent interface continuation is now broader non-empty-interface groundwork or another package/runtime seam that benefits from the explicit type-switch path.
