...
- The new frame-level defer model is now in place; if unwind semantics are the next priority, extend it deliberately into `panic` / `recover` rather than reworking ordinary return handling again.
